...
*/
	...
	__device__ float simplexNoise(float3 pos, float scale, unsigned int seed)
	{
	...
	// Checker pattern
	__device__ float checker(float3 pos, float scale, unsigned int seed)
	{
	...
	__device__ float spots(float3 pos, float scale, unsigned int seed, float size, int minNum, int maxNum, float jitter, profileShape shape)
	{
		if (size < EPSILON)
			return 0.0f;
	...
	float genWorleyNoise(float3 pos, float scale, unsigned int seed, int minNum, int maxNum, float jitter) {
		int ix = (int)(pos.x * scale);
		int iy = (int)(pos.y * scale);
		int iz = (int)(pos.z * scale);
	...
	__device__ __forceinline__
	float worleyNoise(float3 pos, float scale, unsigned int seed, int minNum, int maxNum, float jitter) {
		return genWorleyNoise<1>(pos, scale, seed, minNum, maxNum, jitter);
	}
	
	/**
		More mathematically beautiful.
	*/
	__device__ __forceinline__
	float worleyNoise_five(float3 pos, float scale, unsigned int seed, int minNum, int maxNum, float jitter) {
		return genWorleyNoise<2>(pos, scale, seed, minNum, maxNum, jitter);
	}
	...
	__device__ float discreteNoise(float3 pos, float scale, unsigned int seed)
	{
		int ix = (int)(pos.x * scale);
		int iy = (int)(pos.y * scale);
		int iz = (int)(pos.z * scale);
	...
	__device__ float cubicValue(float3 pos, float scale, unsigned int seed)
	{
		pos.x = pos.x * scale;
		pos.y = pos.y * scale;
		pos.z = pos.z * scale;
	...
	#define MAKE_REPEATER( NAME , FUNC )\
	__device__ float NAME(float3 pos, float scale, unsigned int seed, int n, float lacunarity, float decay) {\
		float acc = 0.0f;\
		float amp = 1.0f;\
		float div = 0.0f;\
		\
	...
	#define MAKE_ABS_REPEATER( NAME , FUNC )\
	__device__ float NAME(float3 pos, float scale, unsigned int seed, int n, float lacunarity, float decay)\
	{\
		float acc = 0.0f;\
		float amp = 1.0f;\
	...
	#define MAKE_FRACTAL_REPEATER( NAME, FUNC )\
	__device__ float NAME(float3 pos, float scale, unsigned int seed, float du, int n, float lacunarity, float decay)\
	{\
		float acc = 0.0f;\
		float amp = 1.0f;\
		float div = 1.0f;\
	...
	#define MAKE_ALL_REPEATERS( NAME , FUNC )\
	MAKE_REPEATER(         repeater##NAME     , FUNC )\
	MAKE_ABS_REPEATER(     repeater##NAME##Abs , FUNC )\
	MAKE_FRACTAL_REPEATER(  fractal##NAME     , FUNC )
	
	MAKE_ALL_REPEATERS(Perlin, perlinNoise)
	MAKE_ALL_REPEATERS(Simplex, simplexNoise)
	...
	__device__ float repeater(float3 pos, float scale, unsigned int seed, int n, float lacunarity, float decay, basisFunction basis)
	{
		float acc = 0.0f;
		float amp = 1.0f;
		float div = 0.0f;
		...
		for (int i = 0; i < n; i++)
		{
			...
			case(BASIS_SIMPLEX):
				...
				break;
			case(BASIS_WORLEY):
				acc += worleyNoise(make_float3(pos.x * scale, pos.y * scale, pos.z * scale), 1.0f, seed, 4, 4, 1.0f) * amp;
				break;
			case(BASIS_SPOTS):
	...
	__device__ float turbulence(float3 pos, float scaleIn, float scaleOut, unsigned int seed, float strength, basisFunction inFunc, basisFunction outFunc)
	{
		switch (inFunc)
		{
		case(BASIS_CHECKER):
		...
		case(BASIS_SIMPLEX):
			...
			break;
		case(BASIS_WORLEY):
			pos.x += worleyNoise(pos, scaleIn, seed ^ 0x1d96f515, 4, 4, 1.0f) * strength;
			pos.y += worleyNoise(pos, scaleIn, seed ^ 0x4df308f0, 4, 4, 1.0f) * strength;
			pos.z += worleyNoise(pos, scaleIn, seed ^ 0x2b79442a, 4, 4, 1.0f) * strength;
			break;
		}
		...
		case(BASIS_WORLEY):
			return worleyNoise(pos, scaleIn, seed, 4, 4, 1.0f);
		}
	...
	__device__ float repeaterTurbulence(float3 pos, float scaleIn, float scaleOut, unsigned int seed, float strength, int n, basisFunction basisIn, basisFunction basisOut)
	{
